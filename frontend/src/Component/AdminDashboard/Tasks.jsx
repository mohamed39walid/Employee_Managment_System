...
function Tasks() {
  ...
  const fetchAndUpdateTasks = async () => {
    try {
      ...
      // Check and update tasks with passed deadlines
      const updatedTasks = res.data.map(async (task) => {
        if (new Date(task.deadline) < now && task.status !== 'completed') {
          ...
          task.status = 'missing'; // Update status locally
        }
        return task;
      });
  ...
  const filteredTasks = tasks.filter((task) => {
    ...
    return (
      (title.includes(query) || name.includes(query) || email.includes(query)) &&
            ...
            {currentTasks.length > 0 ? (
              currentTasks.map((task) => (
                <tr key={task._id}>
                  ...
                  <td className="py-2 px-4 border-b border-gray-200">
                    {task.employee_id?.user_id.name || "N/A"}
                  </td>
                  <td className="py-2 px-4 border-b border-gray-200">
                    {task.employee_id?.user_id.email || "N/A"}
                  </td>
                  <td className="py-2 px-4 border-b border-gray-200">
